...
(this.parse=c);this.clrs=d;this.dflt=g;this.pr=q||0},L=x._registerComplexSpecialProp=function(a,g,c,b,d,m,q){for(var a=a.split(","),g=g instanceof Array?g:[g],j=a.length;-1<--j;)new oa(a[j],g[j],c,b&&0===j,d,m,q)},x=function(a){if(!v[a]){var g=a.charAt(0).toUpperCase()+a.substr(1)+"Plugin";L(a,null,function(a,c,b,d,m,q,j){var e=window.com.greensock.plugins[g];if(!e)return window.console&&console.log("Error: "+g+" libs file not loaded."),m;e._cssRegister();return v[b].parse(a,c,b,d,m,q,j)})}},b=oa.prototype;
